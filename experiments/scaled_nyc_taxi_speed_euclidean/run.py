...
# Re-use loaders from the standard NYC taxi experiment
...
from spef_ot import ScalingMatchResult, scaling_match  # noqa: E402
...
class ExperimentConfig:
    ...
    k: int = 512
    # In scaling context, 'delta' is the FINAL target delta
    delta: float = 0.001
    initial_delta: float | None = None  # Optional start point for scaling
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="NYC taxi SCALED experiment using Euclidean speed kernel and Epsilon Scaling",
        epilog="Runs the scaling solver to iteratively refine epsilon."
    )
    ...
    parser.add_argument("--delta", type=float, default=None, help="Final target Scaling delta")
    parser.add_argument("--initial-delta", dest="initial_delta", type=float, default=None, help="Starting delta for scaling (default: target * 16)")
...
) -> tuple[ScalingMatchResult, float]:
    if device.type == "cuda":
        torch.cuda.synchronize()
    t0 = time.perf_counter()
    
    # CALLING THE NEW SCALING MATCH FUNCTION
...
def main() -> None:
    ...
    log("=== NYC Taxi SCALED Euclidean Speed Experiment ===")
    ...
    log(f"Target Delta    : {config.delta}")
    log(f"Initial Delta   : {config.initial_delta}")
    log(f"Device          : {device}")
    ...
    def progress_callback(event: str, payload: dict[str, Any]) -> None:
        if event == "iteration":
            # Modified callback to show Phase info from scaling solver
            print(
                f"[Phase {payload.get('phase', '?')}] [Iter {payload['iteration']}] "
    ...
    print(f"\nScaled run completed in {runtime:.4f} s")
    print(f"Total Iterations: {int(result.iterations)}")
    print(f"Total Scaling Phases: {int(result.phases)}")
    ...
    output = {
        "params": {
            "experiment_type": "scaled_euclidean_speed",
        ...
        "performance": {
            "runtime_sec": runtime,
            "total_phases": int(result.phases),
