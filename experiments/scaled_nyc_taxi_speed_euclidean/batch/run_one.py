...
def _run_name_from_config(cfg: Dict[str, Any]) -> str:
    ...
    delta = cfg.get("delta")
    initial_delta = cfg.get("initial_delta")
    ...
    if initial_delta is not None:
        parts.append(f"d0{_fmt_float_compact(float(initial_delta))}")
...
def _build_cmd(cfg: Dict[str, Any], out_path: Path) -> list[str]:
    ...
    # Map config params to run.py flags
    ...
    add("--initial-delta", cfg.get("initial_delta"))
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run a single scaled NYC Euclidean Speed experiment from JSON config")
