#!/usr/bin/env python3
"""Batch job dispatcher — NYC taxi exact comparison experiment.

Each SLURM job calls this script with a single JSON config file.
It dispatches to run_spef.py or run_ortools.py based on the "solver" field,
...
RUN_ORTOOLS_PY = EXP_DIR / "run_ortools.py"
...
def _build_ortools_cmd(cfg: Dict[str, Any], out_path: Path) -> list[str]:
    cmd = [sys.executable, str(RUN_ORTOOLS_PY)]
...
def run_once(
    ...
    if solver not in ("spef_unscaled", "spef_scaled", "ortools"):
    ...
    if solver in ("spef_unscaled", "spef_scaled"):
        ...
        cmd = _build_ortools_cmd(cfg, artifact_tmp)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run one NYC taxi exact-comparison job from a JSON config")
