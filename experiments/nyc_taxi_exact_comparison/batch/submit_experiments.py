...
Run from the repo root on the cluster login node:
  python experiments/nyc_taxi_exact_comparison/batch/submit_experiments.py
...
Submit only the two SPEF solvers first (fast), hold ortools for later:
  python experiments/nyc_taxi_exact_comparison/batch/submit_experiments.py --solvers spef_unscaled spef_scaled

Submit only n=1000,2000 as a quick sanity check:
  python experiments/nyc_taxi_exact_comparison/batch/submit_experiments.py --n 1000 2000

Dry-run (print sbatch commands without running them):
  python experiments/nyc_taxi_exact_comparison/batch/submit_experiments.py --dry-run
"""
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Submit exact-comparison sbatch jobs to SLURM")
    ap.add_argument(
        "--solvers", nargs="+",
        choices=["spef_unscaled", "spef_scaled", "ortools"],
        default=None,
        help="Restrict to these solvers (default: all three)",
    )
    ap.add_argument(
