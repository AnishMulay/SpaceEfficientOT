...
def test_matching_algorithms():
    ...
    print(f"Testing with n={n}, d={d}, delta={delta}")
    ...
    xa_cpu = xa.cpu().numpy()
    xb_cpu = xb.cpu().numpy()
    W = cdist(xb_cpu, xa_cpu, 'sqeuclidean')  # Note: xb first, xa second for correct dimensions
    W_tensor = torch.tensor(W, device=device, dtype=torch.float32)
    C = torch.tensor(W.max(), device=device, dtype=torch.float32)
    delta_tensor = torch.tensor(delta, device=device, dtype=torch.float32)
    ...
    Mb, yA, yB, matching_cost, iteration = matching_torch_v1(W_tensor, C, delta_tensor, device)
    ...
    spef_Mb, spef_yA, spef_yB, spef_cost, spef_iteration = spef_matching_torch(xa, xb, C, delta_tensor, device)
    print(f"SPEF matching completed in {spef_iteration} iterations")
    print("SPEF matching initialization completed")
